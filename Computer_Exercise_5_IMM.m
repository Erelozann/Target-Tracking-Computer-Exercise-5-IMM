...
% Filter Parameters
IMM_Filter_Parameters(1,1).H=  H;                      
IMM_Filter_Parameters(1,1).Q = Q;
IMM_Filter_Parameters(1,1).R = R;
IMM_Filter_Parameters(1,1).T = T;

IMM_Filter_Parameters(1,2).H=  H;                     
IMM_Filter_Parameters(1,2).Q = Q;
IMM_Filter_Parameters(1,2).R = R;
IMM_Filter_Parameters(1,2).T = T;

IMM_Filter_Parameters(2,1).H=  H;                      
IMM_Filter_Parameters(2,1).Q = Q;
IMM_Filter_Parameters(2,1).R = R;
IMM_Filter_Parameters(2,1).T = T;

IMM_Filter_Parameters(2,2).H=  H;                     
IMM_Filter_Parameters(2,2).Q = Q;
IMM_Filter_Parameters(2,2).R = R;
IMM_Filter_Parameters(2,2).T = T;
% Estimate Initialization
...
for MC = 1:MC_Num
    for i=1:Step_Num-1
        ...
        for k=1:2
         ...
         % Karistirma olasiliklari hesaplaniyor:
        %----------------------------------------
        [~, c, Mij] = immKaristirmaOlasiligiHesapla(TransProb, T, Local_Trackers(k).Mu(:,i));
...
optim_param = 20;
w = linspace(1/optim_param,1-1/optim_param,optim_param-1);
    ...
    for i=1:Step_Num-1
        if  i~=1 && mod(i,2) == 1 
            ...
            for l=1:Model_Num
                ...
                [FusedCovarianceTrace,FusedModeProb,FusedModeCovariance,FusedModeEstimate] = SPCF(CI_Local_Trackers(2).Mu(l,i), CI_Local_Trackers(2).StateEstimates(:,l,i), CI_Local_Trackers(2).StateEstimatesCov(:,:,l,i), CI_Local_Trackers(1).Mu(:,i),CI_Local_Trackers(1).StateEstimates(:,:,i), CI_Local_Trackers(1).StateEstimatesCov(:,:,:,i), w(1));
                ...
                for m=2:optim_param-1
                
                [FusedCovarianceTrace,FusedModeProb,FusedModeCovariance,FusedModeEstimate] = SPCF(CI_Local_Trackers(2).Mu(l,i), CI_Local_Trackers(2).StateEstimates(:,l,i), CI_Local_Trackers(2).StateEstimatesCov(:,:,l,i), CI_Local_Trackers(1).Mu(:,i),CI_Local_Trackers(1).StateEstimates(:,:,i), CI_Local_Trackers(1).StateEstimatesCov(:,:,:,i), w(m));
                    if minFusedCovarianceTrace > FusedCovarianceTrace
        ...
        for k=1:2
            ...
            % Karistirma olasiliklari hesaplaniyor:
            %----------------------------------------
            [~, c, Mij] = immKaristirmaOlasiligiHesapla(TransProb, T, CI_Local_Trackers(k).Mu(:,i));
